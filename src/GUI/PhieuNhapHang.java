...
public class PhieuNhapHang extends BasePanel {
    ...
    private void initAdvancedFilters() {
        ...
        lblMinAmount.setBounds(20, yPos, 80, 25); // Tăng width để phù hợp với text mới
        add(lblMinAmount);

        txtMinAmount = new JTextField();
        txtMinAmount.setBounds(100, yPos, 120, 25); // Điều chỉnh vị trí và kích thước
        add(txtMinAmount);

        JLabel lblMaxAmount = new JLabel("Tổng tiền đến:");
        lblMaxAmount.setBounds(240, yPos, 100, 25); // Thay đổi text và kích thước
        add(lblMaxAmount);

        txtMaxAmount = new JTextField();
        txtMaxAmount.setBounds(360, yPos, 120, 25); // Điều chỉnh vị trí
        add(txtMaxAmount);
    }
    ...
    private void performSearch() {
        ...
        try {
            if (dateChooserFrom.getDate() != null) {
                fromDate = sdf.format(dateChooserFrom.getDate()); //Chuyển ngày thành chuỗi dd-MM-yyyy
            }
        ...
        try {
            if (!txtMinAmount.getText().isEmpty()) {
                minAmount = Double.parseDouble(txtMinAmount.getText()); //Chuyển chuỗi thành số
            }
