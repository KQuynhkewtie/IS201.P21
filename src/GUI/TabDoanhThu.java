...
public class TabDoanhThu extends JPanel {
    ...
    private void initUIComponents() {
        ...
        btnSearch.addActionListener(e -> KiemTraDuLieuHopLe());
        add(btnSearch);
    ...
    private void KiemTraDuLieuHopLe() {
        String input = dayField.getText().trim();

        try {
            if (rbDay.isSelected()) {
                TraCuuDoanhThuTheoNgay(input);
            } else if (rbMonth.isSelected()) {
                TraCuuDoanhThuTheoThang(input);
            } else if (rbYear.isSelected()) {
                TraCuuDoanhThuTheoNam(input);
    ...
    private void TraCuuDoanhThuTheoNgay(String dateStr) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        Date date = sdf.parse(dateStr);

        List<ThongKeDTO> data = thongKeBLL.TraCuuDoanhThuTheoNgay(date);

        barChart.setVisible(false);

        HienThiDSHoaDon(data, "dd/MM/yyyy");

        CapNhatTongDoanhThu(data);
    }

    private void TraCuuDoanhThuTheoThang(String monthYear) throws ParseException {
        List<ThongKeDTO> tableData = thongKeBLL.TraCuuDoanhThuTheoThang(monthYear);
        ...
        HienThiDSHoaDon(tableData, "dd/MM");

        CapNhatTongDoanhThu(tableData);
    }

    private void TraCuuDoanhThuTheoNam(String year) {
        List<ThongKeDTO> tableData = thongKeBLL.TraCuuDoanhThuTheoNam(year);
        double[] chartData = thongKeBLL.getChartDataForYear(year);

        barChart.updateMonthChart(year, chartData);
        barChart.setVisible(true);

        HienThiDSHoaDon(tableData, "MM/yyyy");

        CapNhatTongDoanhThu(tableData);
    }

    private void TraCuuDoanhThuTong() {
        String currentYear = new SimpleDateFormat("yyyy").format(new Date());

        List<ThongKeDTO> tableData = thongKeBLL.TraCuuDoanhThuTong(currentYear);
        double[] chartData = thongKeBLL.getChartDataForTotal(currentYear);

        barChart.updateYearChart(currentYear, chartData);
        barChart.setVisible(true);

        HienThiDSHoaDon(tableData, "yyyy");

        CapNhatTongDoanhThu(tableData);
    }

    private void HienThiDSHoaDon(List<ThongKeDTO> data, String dateFormat) {
    ...
    private void CapNhatTongDoanhThu(List<ThongKeDTO> data) {
